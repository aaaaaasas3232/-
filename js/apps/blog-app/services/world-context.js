...
/** 用户在氧气里的社媒形象（nook 人设卡「社媒形象 → 氧气」配置的） */
export function readUserSocialProfile(user) {
    const u = user || getDefaultUser();
    const profile = u?.socialProfiles?.blog || {};
    return {
...
export async function listLibraryPrompts() {
    ...
    try {
        ...
        console.warn('[blog] 读取 prompt 库失败', err);
        return [];
    }
}
...
export function listWorldAis(world) {
    ...
    return listWorldAiPersons(w, s).map((ai) => ({
        ...
        tone: String(ai?.tone || ''),
        social: ai?.socialProfiles?.blog || null,
    }));
}
...
/**
 * 把「TA 在氧气发过什么帖子」写进 AI 的人设经历区（幂等）。
 * murmur 拼人设时会带上 experience，AI 聊天时就记得自己发过这条帖子。
 */
export async function appendAiExperience(aiId, line) {
    ...
    try {
        ...
        console.warn('[blog] 写 AI 经历失败', err);
        return false;
    }
}
