 ...
 * 生成 JSON+yaml+md 融合的紧凑格式，键无引号，值按需加引号。
 */

/**
 * 从 persona 动态生成上下文文本（忽略 customContext）
 * 用于"恢复默认"功能
 * 生成 md+yaml+json 混合格式，便于 AI 阅读
 */
export function buildContextFromPersona(persona, entityType) {
    ...
    if (persona.age != null) basicFields.push(`age: ${persona.age}`);
    if (persona.identity) basicFields.push(`identity: ${persona.identity}`);
    ...
    // 5. 人生履历 (Timeline) - 从 world 的 timelines 里获取
    let timelineData = persona.lifePhases; // 回退用 persona.lifePhases
    ...
    // 9. 资产信息 —— 余额 / 收入 默认进入上下文；资产说明（金钱观 / 经济处境）
//    受 persona.assetNotes.enabled 开关控制
    ...
    if (hasBalance || hasIncome || hasAssetNote) {
        ...
        if (hasBalance) {
            ...
            // 货币单位：表单字段实际是 `name`（不是 `unit`）。优先用 name，回退 symbol。
    ...
    // 10. 今日心情（injectMode: none | current）
    const moodInject = persona?.mood?.injectMode || 'none';
    if (moodInject !== 'none') {
        const entityId = persona.id;
        const todayData = sdk?.diary?.getToday?.(entityType, entityId);
    ...
    // 11. 本周日程（injectMode: none | current | full）
    ...
    if (schedInject !== 'none') {
        ...
        if (schedInject === 'current') {
            ...
            // nearby: 注入昨天 / 今天 / 明天的日程（其它天 AI 自称忘了）
            const nearbyDays = [];
    ...
    // 12. 作息（injectMode: none | current | full）
