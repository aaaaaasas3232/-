 ...
 * 用户没绑就退到第一个可用的 —— 「没绑就报错」很难受，
 * 而且那种错误提示不说该去哪儿配。
 *
 * ── 为什么不用流式 ────────────────────────────────────────────────
 *
 * 这个 App 要的都是**结构化 JSON**（职位列表、职位详情、HR 人设、小剧场），
 * 流式吐一半的 JSON 解析不了，展示价值为零。
 * 只有面试对话是纯文本，但它一次就一两句，流式没有意义。
 */
...
/**
 * HTTP 状态码翻译成人话。
 * 面向用户的错误要同时满足：能搜索、能转发、能执行。这里至少做到最后一条。
 */
 ...
 * @param {string} [opts.user]      本轮指令
 * @param {Array}  [opts.history]   多轮对话（面试用）
...
export async function generateText(opts = {}) {
    const {
        system = '', user = '按上面的要求生成。', history = [],
        temperature = 0.9, apiRef: override, signal,
    } = opts;
    ...
    try {
        ...
        //   所以这里一定要 console.error 出来
        console.error('[job] AI 调用抛异常', err);
