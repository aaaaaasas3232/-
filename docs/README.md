...
## 后续卡片类型增补索引（2026-08-15 起）

chat-app 的分享卡类型在本文档定稿后仍在增长，新类型不再回填本文件，统一看：

- 音乐三卡 / 四叶草三卡：`docs/AGENTS2.md` §5、§16
- **萤火视频卡（`youtube_video_share`，2026-08-15）**：`docs/README2.md` 末尾
  「萤火视频卡接入」+ `docs/AGENTS.md §49` + `docs/AGENTS2.md §18`
- 三处对齐口诀（写入 type / message-renderer 注册表 / share-cards 渲染器）永远有效；
  跨 App 内容卡一律走 `createContentCardAction` 确认协议，不写自定义 click handler
